...
const generateRoad_map = async(req, res)=>{ 
    try{
        ...
        const { company, userQuery } = req.body;
        const roadmap = await generateRoadmap(company, userQuery);
         const end = Date.now()
         const responseTime = (end - start) / 1000;
        res.json({ success: true, roadmap, responseTime, queryLength : userQuery.split(" ").length });
    }
    catch(err){
      ...
      .insert([
        {
          ...
          roadmap: processedRoadmap.roadmap
        }
      ])
      .select();
